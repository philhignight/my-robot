...
} = require("./prompt-budget-allocator");
...
async function buildPrompt(extractionMode = false) {
  try {
    ...
    const discoveries = await utils.readFileIfExists(
      "ai-managed/discoveries.md"
    );
    const explorationFindings = await utils.readFileIfExists(
      "ai-managed/exploration-findings.md"
    );
    const detailedPlan = await utils.readFileIfExists(
    ...
    if (historyIndex !== -1) {
      ...
      conversationHistory = conversationHistory
        ...
        .filter(function (line) {
          return (
            !line.includes("=== AI RESPONSE READY ===") &&
    ...
    console.log(
      "📊 Budget usage: " +
        allocation.totalUsed.toLocaleString() +
        " / " +
        allocation.budget.toLocaleString() +
        " characters (" +
        Math.round((allocation.totalUsed / allocation.budget) * 100) +
        "%)"
    );
      ...
      if (alloc.excluded && alloc.excluded.length > 0) {
        console.log(
          "  ⚠ " +
            category +
            ": included " +
            alloc.content.length +
            " chars, excluded " +
            alloc.excluded.length +
            " items"
        );
      }
    }
...
async function buildExtractionPrompt(tempBlock) {
  try {
    ...
    const blockNumber =
      allTempBlocks.findIndex((b) => b.startLine === tempBlock.startLine) + 1;
    ...
    if (isSingleFile) {
      ...
      const hasDocExtension = docExtensions.some((ext) =>
        tempBlock.title.toLowerCase().includes(ext)
      );
      if (hasDocExtension) {
        isCode = false;
      }
...
6) This is block ${blockNumber} of ${totalBlocks} - there ${
      totalBlocks > 1 ? "are more blocks to process" : "is only this block"
    }
...
`;
    ...
    for (let i = conversationLines.length - 1; i >= 0; i--) {
      ...
      if (
        line.includes("└─") &&
        i > 0 &&
        conversationLines[i - 1].includes("│")
      ) {
        inAssistantBox = true;
...
`;
      ...
      const fileName = tempBlock.title
        .replace("Content of ", "")
        .replace("TEMPORARY: ", "")
        .trim();
...
async function processResponse() {
  try {
    ...
    try {
      ...
      if (
        formatError.message.includes("complete ASCII box") &&
...
async function handleExtractionResponse(responseText, tempBlock) {
  try {
    ...
    const summaryBlock = formatSystemToolResult(
    ...
    await handleFatalError(
      "Failed to process extraction response: " + err.message
    );
  }
}
...
async function handleReadResponse(responseText, tools) {
  ...
  for (let i = 0; i < tools.length; i++) {
    ...
    if (tool.name === "READ_CODE" || tool.name === "READ_REQUIREMENTS") {
      ...
      if (typeof rawResult === "string" && rawResult.startsWith("Error:")) {
        ...
        rawResult.endsWith(" is empty") ||
        rawResult.includes("[Empty directory]")
      ) {
        ...
        if (
          ...
          lines[0].startsWith("Contents of ")
        ) {
          fileName = lines[0]
            .replace("Content of ", "")
            .replace("Contents of ", "")
  ...
  if (allResults.length > 0) {
    ...
    for (let i = 0; i < temporaryBlocks.length; i++) {
      ...
      const tempBlock = utils.formatTemporaryBlock(
        block.title,
        block.content,
        blockNumber,
        totalBlocks
...
async function handleWriteResponse(responseText, tools) {
  ...
  const specialTools = tools.filter(function (t) {
    return (
      t.name === "DISCOVERED" ||
      t.name === "SWITCH_TO" ||
      t.name === "SWITCH_TO_EXPLORATION" ||
      t.name === "SWITCH_TO_PLANNING" ||
      t.name === "SWITCH_TO_IMPLEMENTATION" ||
      t.name === "EXPLORATION_FINDINGS" ||
      t.name === "DETAILED_PLAN"
    );
  });
...
async function handleIncompleteMessage(responseText) {
  ...
  const continuationRequest =
    'SYSTEM: Your message was cut off. Please continue from: "' +
    lastPart +
    '"';
  ...
  try {
    ...
    if (stats.isDirectory()) {
      ...
      return await executeListDirectory({
        path: params.file_name || params.path || ".",
      });
    } else {
      // Execute file reading
      return await executeReadFile({
        file_name: params.file_name || params.path,
      });
    }
  } catch (err) {
    ...
    if (err.code === "ENOENT") {
      return (
        "Error: " + (params.file_name || params.path || ".") + " not found"
      );
    }
    return (
      "Error reading " +
...
async function executeReadFile(params) {
  try {
    ...
    const binaryExtensions = [
      ...
      ".sqlite3",
      ".min.js",
      ".min.css", // Often minified files are too large/unreadable
    ];
    ...
    if (binaryExtensions.includes(ext)) {
      return (
        "Error: Cannot read " +
        params.file_name +
        " - AI is not allowed to read non-text files. Binary and compressed files are not supported."
      );
    }
    ...
    if (
      ...
      err.toString().includes("Invalid character")
    ) {
      return (
        "Error: Cannot read " +
        params.file_name +
        " - file appears to be binary. AI is only allowed to read text files."
      );
    }
...
async function handlePendingUpdate(responseText) {
  ...
  if (!utils.hasBoxClosure(responseText)) {
    console.log(
      "⚠ Incomplete message during pending update, requesting continuation"
    );
    await handleIncompleteMessage(responseText);
    return;
  }
  ...
  const tools = utils.parseToolBlocks(responseText);
  const commitTool = tools.find(function (t) {
    return t.name === "COMMIT";
  });
  ...
  if (commitTool || responseText.includes("COMMIT")) {
    ...
    await updateConversation(
      "SYSTEM: Changes committed to " + pendingData.file + " and pushed to git"
    );
    await fs.writeFile("ai-response.md", "", "utf8");
    ...
    if (
      ...
      await updateConversation(
        "SYSTEM: Invalid response. Please use [COMMIT] to apply changes."
      );
...
async function updateConversation(newMessage) {
  ...
  if (newMessage.startsWith("SYSTEM:")) {
    ...
    const isToolResult =
      newMessage.includes("Tool result (") ||
...
function formatSystemToolResult(message) {
  ...
  const isSearchResult =
    title.includes("Content matches") || title.includes("Files found");
  ...
  for (let i = 0; i < contentLines.length; i++) {
    ...
    if (
      ...
      !isReadSummary &&
      actualLineNumber > 50 &&
      (actualLineNumber - 1) % 50 === 0
    ) {
      const fileName = title
        .replace("Content of ", "")
        .replace("Tool result (READ)", "")
        .trim();
      const endLine = Math.min(
        actualLineNumber + 49,
        actualLineNumber + contentLines.length - i - 1
      );
      const contTitle =
        " SYSTEM: Continuing " +
        fileName +
      ...
      if (contTitle.length > boxWidth - 2) {
        ...
        result +=
          "╠═" + shortContTitle + "═".repeat(Math.max(0, contPadding)) + "═╣\n";
      } else {
        const contPadding = boxWidth - contTitle.length - 2;
        result +=
          "╠═" + contTitle + "═".repeat(Math.max(0, contPadding)) + "═╣\n";
      }
    }
  ...
  discoveries: {
    ...
    summarizer: (excluded) => {
      ...
      if (groups.important.length > 0) {
        ...
        groups.important
          .slice(0, 2)
          .forEach((d) => (summary += `- ${d.content.substring(0, 60)}...\n`));
      }
...
function allocatePromptBudget(inputs, mode, currentDate) {
  ...
  for (const category of prioritizedCategories) {
    ...
    let units = config.selector
      ? config.selector(content, currentDate)
      : [content];
  ...
  return {
    allocations,
    totalUsed: Object.values(allocations).reduce(
      (sum, a) => sum + a.content.length + a.summary.length,
      0
    ),
    budget: budget,
  };
...
function generatePrompt(options) {
  ...
  if (!MODE_CONFIGS[mode]) {
    throw new Error(
      "Invalid mode: " +
        mode +
        ". Must be exploration, planning, or implementation"
    );
  }
...
function validateResponseType(text, tools) {
  ...
  if (!hasBox) {
    return {
      valid: false,
      error:
        "Response must be wrapped in ASCII box starting with ┌─ ASSISTANT ─",
    };
  }
...
function validateMessageFormat(text) {
  if (!text.includes("┌─ ASSISTANT") || !hasBoxClosure(text)) {
    throw new Error(
      "Invalid format: Response must be wrapped in complete ASCII box"
    );
  }
...
function compactConversation(conversationHistory, maxLength) {
  ...
  for (let i = 0; i < oldExchanges.length; i++) {
    ...
    if (assistantText.includes("[DISCOVERED]")) {
      const discoveryMatch = assistantText.match(
        /\[DISCOVERED\]\s+(\d+)\s+([^\n]+)/
      );
      if (discoveryMatch) {
        discoveries.push(
...
function calculatePromptLength(
  basePrompt,
  modePrompt,
  goals,
  context,
  ...
  return (basePrompt + modePrompt + goals + context + conversationHistory)
    .length;
}
